...
std::vector<float> Interface::ReadWaveform(float yoffset) noexcept
{
  assert(_isConnected);     
  ...
  // // Get the yoffset to help calculate the vertical values. by Ren
  // command = "CH" + ch + "Offset?\n";
  // status = viQueryf(_instrumentSession,command.c_str() , "%f", &yoffset);
  // printf("YOFF: %f\n",yoffset);
  // if (status < VI_SUCCESS) goto error;
  ...
  for(size_t i = offset; i < recordLen + offset; i++){
    ...
    form.push_back((((double) _buffer[i])) * ymult + yoffset);
  }
